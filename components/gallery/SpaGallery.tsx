        ...
        <div className="flex flex-col sm:flex-row sm:items-end justify-between mb-10 sm:mb-14 gap-6">
          ...
          <div className="hidden sm:flex items-center gap-3 flex-shrink-0">
            <button
              type="button"
              onClick={scrollLeft}
              className="p-3.5 rounded-xl bg-surface-raised border border-rose/20 hover:border-rose text-cream hover:text-rose transition-colors"
          ...
          {galleryData.map((item, idx) => {
            ...
            return (
              <motion.div
                ...
                className={`relative flex-shrink-0 ${widthClass} h-[340px] sm:h-[420px] rounded-2xl overflow-hidden border border-rose/15 hover:border-rose/40 group cursor-pointer shadow-xl bg-surface-raised`}
              >
                <Image
                  src={item.image}
                  alt={item.title}
                ...
                {/* Hover Expand Icon */}
                <div className="absolute top-4 right-4 p-2.5 rounded-xl bg-obsidian/85 backdrop-blur-md border border-rose/25 text-cream opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                  <Maximize2 className="w-4 h-4 text-rose" />
                </div>

                {/* Bottom Caption Overlay */}
                <div className="absolute bottom-5 left-5 right-5">
                  <span className="text-[10px] uppercase tracking-[0.2em] font-sans font-bold text-rose block mb-1">
                    {item.category}
                  </span>
